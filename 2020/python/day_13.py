...
ids = list(map(int, filter(lambda id: id != 'x', raw_ids)))


def find_next_departure(ids, time):
    departures = list(map(lambda id: (id, (time / id + 1) * id), ids))
    return min(departures, key=lambda d: d[1])


def solve_1(ids, time):
    (line, departure) = find_next_departure(ids, time)
    return line * (departure - time)


# print("2020 - Day 13 - Part 1")
# print(solve_1(ids, time))
# => 115
#    ===
...
indices = list(filter(lambda i: i >= 0, list(
    map(lambda x: x[0] if x[1] != 'x' else -1, enumerate(raw_ids)))))

print("\n2020 - Day 13 - Part 2")
print(chinese_remainder(ids, list(map(lambda i: -i, indices))))
# => 756261495958122
#    ===============
