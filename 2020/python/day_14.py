...
def write_masked_values(instructions):
    memory = {}
    cur_masks = (0, 0)
    ...
    for instr in instructions:
        op = instr[:3]
        if op == 'mas':
            cur_masks = create_value_masks(mask_pattern.match(instr).group(1))
        elif op == 'mem':
            groups = mem_pattern.match(instr).groups()
            (address, value) = (int(groups[0]), int(groups[1]))
            memory[address] = (value & cur_masks[0]) | cur_masks[1]

    return memory


print("2020 - Day 14 - Part 1")
print(sum(write_masked_values(instructions).values()))
# => 6317049172545
#    =============
...
def get_masked_addresses(address, mask):
    ...
    for i, c in enumerate(mask):
        if c == '1':
            address |= 1 << (35 - i)
        if c == 'X':
            floating_bits.append(1 << (35 - i))

    floating_variations = []
    for i in range(len(floating_bits) + 1):
        floating_variations.extend(
            itertools.combinations(floating_bits, i))

    masks = [sum(variation) for variation in floating_variations]
    return [address ^ mask for mask in masks]


def write_to_masked_addresses(instructions):
    memory = {}
    cur_mask = ''
    ...
    for instr in instructions:
        op = instr[:3]
        if op == 'mas':
            cur_mask = mask_pattern.match(instr).group(1)
        elif op == 'mem':
            groups = mem_pattern.match(instr).groups()
            (address, value) = (int(groups[0]), int(groups[1]))
            addresses = get_masked_addresses(address, cur_mask)
...
print("\n2020 - Day 14 - Part 2")
print(sum(write_to_masked_addresses(instructions).values()))
# => 3434009980379
#    =============
