...
INIT_NUM = 7
TRANSFORM_SECRET = 20201227


def read_public_keys(f) -> Tuple[int, int]:
    lines = f.read().split('\n')
...
# with open(os.path.dirname(__file__) + "/../examples/example_25.txt") as f:
with open(os.path.dirname(__file__) + "/../inputs/input_25.txt") as f:
    card_pub, door_pub = read_public_keys(f)
    card_loop = find_loop_size(INIT_NUM, card_pub)
    door_loop = find_loop_size(INIT_NUM, door_pub)
    card_enc = transform(card_pub, door_loop)
    door_enc = transform(door_pub, card_loop)
    print("2020 - Day 25 - Part 1")
    print('(card, door)')
    print('pub-keys:', (card_pub, door_pub))
    print('loop-size:', (card_loop, door_loop))
    print('enc-key:', (card_enc, door_enc))
    # => 15217943
    #    ========
