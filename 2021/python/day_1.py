...
# with open(os.path.dirname(__file__) + "/../examples/example_1.txt") as f:
with open(os.path.dirname(__file__) + "/../inputs/input_1.txt") as f:

    depthValues = list(map(int, f.readlines()))

    print("2021 - Day 1 - Part 1")
    print("How many measurements are larger than the previous measurement?")
    print(count_increases(depthValues))
    # => 1120
    #    ====
...
def sum_window_values(arr: List[int], window_size: int) -> List[int]:
    sums = []
    for i in range(max(len(arr) - (window_size - 1), 1)):
        sums.append(sum(arr[i:i+min(len(arr)-i, window_size)]))
    return sums
...
with open(os.path.dirname(__file__) + "/../inputs/input_1.txt") as f:
    ...
    print("2021 - Day 1 - Part 2")
    print("How many sums are larger than the previous sum?")

    sums = sum_window_values(depthValues, 3)
