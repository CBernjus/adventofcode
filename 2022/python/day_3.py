...
# Part 1: Rucksack Reorganisation
# -------------------------------
...
def map_codes_for(item_types: List[str]) -> List[int]:
    mapped_codes = []
...
def find_wrong_item_types(rucksacks: List[Tuple[str, str]]) -> List[str]:
    wrong_types = []
    for (first, second) in rucksacks:
        tmp = set(list(first))
        for c in second:
            if c in tmp:
                wrong_types.append(c)
                break
    return wrong_types


def list_compartments(file: TextIO) -> List[Tuple[str, str]]:
    lines = list(file.read().split('\n'))
    return [(line[:len(line) // 2], line[len(line) // 2:]) for line in lines]


# with open(os.path.dirname(__file__) + "/../examples/example_3.txt") as f:
with open(os.path.dirname(__file__) + "/../inputs/input_3.txt") as f:
    rucksack_compartments = list_compartments(f)
    types = find_wrong_item_types(rucksack_compartments)
    item_codes = map_codes_for(types)

    print("2022 - Day 3 - Part 1")
    print(sum(item_codes))
    # => 7889
    #    ====
...
def get_badge_codes(groups: List[Tuple[Set[str], Set[str], Set[str]]]) -> List[int]:
    badges = [list(a.intersection(b, c))[0] for a, b, c in groups]
    return map_codes_for(badges)


def list_groups(file: TextIO) -> List[Tuple[Set[str], Set[str], Set[str]]]:
    lines = file.read().split('\n')
    iterator = iter(lines)
    return [(set(a), set(b), set(c)) for a, b, c in zip(iterator, iterator, iterator)]


# with open(os.path.dirname(__file__) + "/../examples/example_3.txt") as f:
with open(os.path.dirname(__file__) + "/../inputs/input_3.txt") as f:
    groups = list_groups(f)
    badge_codes = get_badge_codes(groups)

    print("2022 - Day 3 - Part 2")
    print(sum(badge_codes))
    # => 2825
    #    ====
