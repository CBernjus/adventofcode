...
def list_pairs(file: TextIO) -> list[list[int]]:
    return [list(map(int, re.split(r'[,-]', line))) for line in file.read().split('\n')]


def fully_contained(pair: list[int]) -> bool:
...
# with open(os.path.dirname(__file__) + "/../examples/example_4.txt") as f:
with open(os.path.dirname(__file__) + "/../inputs/input_4.txt") as f:
    pairs = list_pairs(f)
    fully_contained_pairs = list(filter(lambda pair: fully_contained(pair), pairs))

    print("2022 - Day 4 - Part 1")
...
def overlapping(pair: list[int]) -> bool:
    start1, end1, start2, end2 = pair
...
with open(os.path.dirname(__file__) + "/../inputs/input_4.txt") as f:
    pairs = list_pairs(f)
    overlapping_pairs = list(filter(lambda pair: overlapping(pair), pairs))

    print("2022 - Day 4 - Part 2")
