...
def read_signals(file: TextIO) -> list[str]:
    return file.read().split('\n')


def find_start_of_packet(signal: str, window_length: int) -> int:
    for i in range(window_length, len(signal)):
        chunk = signal[i-window_length:i]
        if len(set(chunk)) == window_length:
            return i
    return -1


def find_for_all(signals: list[str], window_length: int) -> list[int]:
    return list(map(lambda signal: find_start_of_packet(signal, window_length), signals))


# with open(os.path.dirname(__file__) + "/../examples/example_6.txt") as f:
with open(os.path.dirname(__file__) + "/../inputs/input_6.txt") as f:
    raw_signals = read_signals(f)
    packet_starts = find_for_all(raw_signals, 4)

    print("2022 - Day 6 - Part 1")
    print(packet_starts[0])
    # => 1876
    #    ====
...
with open(os.path.dirname(__file__) + "/../inputs/input_6.txt") as f:
    raw_signals = read_signals(f)
    message_starts = find_for_all(raw_signals, 14)

    print("2022 - Day 6 - Part 2")
    print(message_starts[0])
    # => 2202
    #    ====
